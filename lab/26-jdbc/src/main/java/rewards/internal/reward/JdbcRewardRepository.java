...
//		return new RewardConfirmation(confirmationNumber, contribution);
		...
		return jdbcTemplate.update(sql, confirmationNumber, contribution.getAmount().asBigDecimal(),
				new Date(SimpleDate.today().inMilliseconds()), contribution.getAccountNumber(),
				dining.getMerchantNumber(), new Date(dining.getDate().inMilliseconds()), dining.getAmount().asBigDecimal()) > 0 ?
				new RewardConfirmation(confirmationNumber, contribution) : null;
	}
